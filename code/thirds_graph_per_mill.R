thirds_graphs_pm <- function(Y, k = 3, labels = FALSE,
                          lab_q_x = 0.95, lab_q_y = 0.95,
                          plot_title = '', df = df_full, add_range = TRUE) {
  ...
  range_list <- lapply(levels(plot_data$my_group), function(x) {
    temp <- plot_data %>% filter(my_group == !!x) %>% 
      ...
      round(2)
    if(add_range)  return(paste0('(', temp[1], ' to ', temp[2], ')'))
    if(!add_range) return(" ")
  ...
  label_data <- left_join(df, parts) %>%
    ...
    mutate(high_x = ifelse(elapsed_rel > quantile(elapsed_rel, lab_q_x, na.rm = T),
                           1, 0)) %>%
    mutate(high_y = ifelse(deaths_cum_per_million > quantile(deaths_cum_per_million, lab_q_y, na.rm = T),
                           1, 0)) %>%
    mutate(sparse = (n() < 20)) %>%
    ungroup() %>%
  ...
  ggplot(plot_data_bw, aes(x=elapsed_rel,
    ...
    geom_text_repel(data = label_data,
                    mapping  = aes(label = geoid2, x = elapsed_rel,
                                  y = deaths_cum_per_million, color = geoid2),
                    nudge_x = 4, segment.color = "darkgrey")  +
